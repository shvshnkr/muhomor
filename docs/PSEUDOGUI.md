...
Интерактивный экран в духе Wails Simple: статус, карточка сервера, кнопка подключения, трафик/таймер, диагностика, футер.

| Клавиша | Действие |
|---------|----------|
| ↑ / ↓ | фокус по элементам |
| Enter | действие (подключить, ping, диагностика, экспорт, расширенный режим) |
| q / Esc | выход (демон остаётся) |

**Расширенный режим** — боковая навигация (↑↓), Enter: «Конфигурация» → `[g]`, «Маршрут» → `[r]`, «Настройки» → `[s]`; Esc — назад на простой экран. Заготовка под полный паритет с `Extended.tsx` в GUI.

Если stdin не TTY, автоматически открывается **классическое меню** (номера 1–8, буквы).

## Классическое меню (fallback)
